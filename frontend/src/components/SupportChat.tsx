...
const SupportChat = () => {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data?.message) {
        setMessages((prev) => [
          ...prev,
          { type: 'bot', text: response.data.message, time: new Date() },
        ]);
      } else {
      ...
      setMessages((prev) => [
        ...prev,
        { type: 'bot', text: errorMessage, time: new Date() },
      ]);
  ...
  return (
    <>
      <button
        onClick={toggleOpen}
        className={`fixed bottom-6 right-6 z-50 w-14 h-14 bg-crimson-red hover:bg-fire-red text-white rounded-full shadow-lg hover:shadow-xl flex items-center justify-center transition-all duration-300 hover-lift ${isOpen ? 'rotate-180' : ''
          }`}
      ...
      {/* Always rendered, toggled via CSS — preserves scroll position and chat history */}
      <div
        ref={chatContainerRef}
        className={`fixed bottom-24 right-6 z-50 w-96 max-w-[calc(100vw-3rem)] h-[500px] max-h-[calc(100vh-8rem)] bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl flex flex-col transition-all duration-200 origin-bottom-right ${isOpen
