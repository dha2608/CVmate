...
const ToastItem = memo(({ toast, onRemove }: { toast: { id: string; type: ToastType; message: string }; onRemove: (id: string) => void }) => {
  ...
  return (
    <motion.div
      ...
      className={`pointer-events-auto ${BG_MAP[toast.type] ?? 'bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-200'} border rounded-lg sm:rounded-xl shadow-lg p-3 sm:p-4 flex items-start gap-2 sm:gap-3 backdrop-blur-sm`}
    >
