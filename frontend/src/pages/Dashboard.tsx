import { lazy, Suspense, useEffect, useMemo, memo, useCallback, useState }from 'react';
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        logger.info('dashboard_loaded', { durationMs: Math.round(performance.now() - start) });
      }catch (error) {
    ...
    <MainLayout>
      <motion.div className="space-y-6" initial={{ opacity: 0, y: 20 }}animate={{ opacity: 1, y: 0 }}transition={{ duration: 0.4 }}>
        <GlassCard className="p-4 sm:p-6" gradient="purple">
          ...
          <div onClick={handleCommunityClick}className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-200 border border-gray-200/50 dark:border-gray-600/50">
          ...
          <GlassCard className="p-4 sm:p-6" gradient="cyan">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <h2 className="text-sm sm:text-base font-bold text-gray-900 dark:text-white">{nextAction.title}</h2>
                <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 max-w-xl">{nextAction.desc}</p>
              </div>
              <div className="flex items-center gap-4 w-full md:w-auto">
                <div className="flex flex-col items-start">
                  ...
                  <div className="w-32 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                    <div className="h-2 bg-crimson-red dark:bg-red-500 rounded-full transition-all duration-500" style={{ width: `${Math.min(100, Math.round(profileProgress))}%` }}/>
                  </div>
                  <span className="text-[11px] font-semibold text-gray-700 dark:text-gray-300 mt-1">{Math.round(profileProgress)}%</span>
                </div>
                <GlassButton size="sm" variant="pink" className="whitespace-nowrap" onClick={() => navigate(nextAction.href)}>
                  {nextAction.cta}
                </GlassButton>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-rose-50 to-orange-50 dark:from-rose-900/20 dark:to-orange-900/20 rounded-lg border border-rose-200 dark:border-rose-800 p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div className="bg-rose-500 dark:bg-rose-600 h-2 rounded-full transition-all duration-500" style={{ width: `${Math.min(100, ((stats.resumesCount || 0) / 10) * 100)}%` }}/>
                </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-1">
            ...
            <Button variant="link" className="text-rose-500 dark:text-rose-400 text-xs sm:text-sm p-0 h-auto font-semibold" onClick={handleBlogClick}>
              {t('dashboard.viewAll')}
            </Button>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <Button onClick={handleBlogClick}variant="outline" className="mt-4">Browse Articles</Button>
            </div>
          )}
        </div>
      </motion.div>
