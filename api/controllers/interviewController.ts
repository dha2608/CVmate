...
> = {
  ...
  'product-management': {
    ...
    followUpStyle:
      'ask about user impact, metrics, prioritization rationale, stakeholder alignment',
    redFlags: [
      'no user focus',
      'weak prioritization',
      'ignores data',
  ...
  sales: {
    ...
    followUpStyle:
      'challenge with objections, ask about closing techniques, probe on relationship building',
  ...
  finance: {
    ...
    followUpStyle:
      'drill into numbers, ask about assumptions, challenge financial logic, probe on risk',
    redFlags: [
      'calculation errors',
  ...
  'data-science': {
    ...
    followUpStyle:
      'drill into methodology, ask about assumptions, challenge statistical reasoning, probe on business value',
...
- If they mention an industry, adapt your questions to that industry's context using relevant industry knowledge`,
    firstMessage:
      "Hi there! I'm Sarah from HR. Thanks for joining me today. To start, could you tell me a little bit about yourself and what brings you here?",
  },
  'strict-manager': {
...
- Maintain professional but slightly skeptical tone - you're testing their knowledge`,
    firstMessage:
      "I'm Mike, the Tech Lead. I've reviewed your CV. Let's get straight to the point. Describe the most complex technical challenge you've faced recently and how you solved it.",
  },
  'english-native': {
...
- Correct mistakes gently and naturally, not in a condescending way`,
    firstMessage:
      "Hello! I'm Alex. We're going to have a casual conversation to practice your English. How has your day been so far?",
  },
  'tech-lead': {
...
- Acknowledge good answers but keep pushing: "Good. Now, what if we add constraint Y?"`,
    firstMessage:
      "Hi, I'm David, Senior Tech Lead. I'll be conducting a technical deep-dive today. Let's start with system design: How would you architect a system that needs to handle 10 million concurrent users?",
  },
  'startup-founder': {
...
- If they're too cautious, push them: "We need to move fast. What's the minimum viable approach?"`,
    firstMessage:
      "Hey! I'm Emma, founder of a fast-growing startup. In our world, things change daily. Tell me about a time you had to pivot quickly or adapt to a completely new situation with limited resources.",
  },
  executive: {
...
- If they answer well, acknowledge but raise the bar: "Good. Now, let's talk about a more complex scenario."`,
    firstMessage:
      "Good morning. I'm Robert, the CEO. I'm looking for leaders who can drive impact. Walk me through a strategic decision you made that significantly affected your organization's bottom line.",
  },
  academic: {
...
- If they answer well, acknowledge but probe deeper: "Interesting. Can you elaborate on the theoretical implications?"`,
    firstMessage:
      "Hello, I'm Dr. Chen. I'm evaluating candidates for our research team. Could you tell me about your research background and the most significant contribution you've made to your field?",
  },
};
...
export const sendMessage = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const historyForAI = [interview.chatHistory[0], ...interview.chatHistory.slice(-10)]
      .filter(Boolean)
      .map((msg) => ({
    ...
    try {
      ...
      const aiResponse =
        completion.choices?.[0]?.message?.content?.trim() || "I'm sorry, I didn't catch that.";
      ...
      if (errorObj.status === 401 || errorObj.statusCode === 401) {
        errorMessage =
          'AI API key is invalid or missing. Please check HF_API_KEY in your .env file.';
        statusCode = 503;
        ...
        errorMessage =
          'AI provider requires payment or has insufficient credits. Please check your Hugging Face billing or quota.';
        statusCode = 402;
...
${conversationText}
    ...
    try {
        ...
        typeof scores.structure === 'number'
          ? Math.round(
              [
                ...
                scores.confidence ?? 0,
                scores.structure ?? 0,
              ].reduce((sum: number, v: number) => sum + v, 0) / 4
            )
          : aiData.score || 0;
      ...
      if (interviewCount === 1) {
        await checkAndAwardAchievement(req.user?._id.toString(), 'complete_interview', {
...
export const getInterviewAnalytics = async (
  ...
  try {
    ...
    const interviews = await Interview.find({ user: userId }).select(
      'persona feedback status createdAt'
    );
    ...
    const averageScore =
      completedSessions > 0
        ? Math.round(
            completed.reduce((sum, i) => sum + (i.feedback?.overallScore ?? 0), 0) /
              completedSessions
          )
        : 0;
    ...
    for (const [persona, agg] of Object.entries(scoresByPersona)) {
      averageScoresByPersona[persona] = agg.count > 0 ? Math.round(agg.total / agg.count) : 0;
    }
